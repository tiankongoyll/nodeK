...
Product.belongsTo(User, {//product由user创建
    constraints: true,//
    onDelete: 'CASCADE'//没有外键依赖
});
User.hasMany(Product);//一个用户有多个商品
User.hasOne(Cart);//一个用户有一个购物车
Cart.belongsTo(User);
Cart.belongsToMany(Product, {//cart多对多product，通过cartItem创建
    through: CartItem
});
...
sequelize.sync().then(//同步数据
    async result => {//同步回调
        ...
        if (!user) {
            ...
            await user.createCart();//创建了关系后就有这个方法了。创建一个购物车
        }
